...
strsql = "SELECT `车位`,COUNT(*) AS COUNT FROM bk GROUP BY `车位` HAVING COUNT>1"
rows = data(strsql)
rows = list(rows)
...
def pie_custom_radius(xdata4, ydata4):
    ...
    pie.add("",
            ...
            # 设置半径范围，0%-100%
            radius=["40%", "75%"])
    pie.set_global_opts(title_opts=opts.TitleOpts(title="车位"), legend_opts=opts.LegendOpts(is_show=False))
    return pie
...
pie_custom_radius(xdata4, ydata4).render("pie_custom_radius3.html")
