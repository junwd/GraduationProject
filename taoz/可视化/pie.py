...
strsql = "SELECT gp.`名称`,gp.`最新价` FROM gp LIMIT 8"
rows = data(strsql)
rows = list(rows)


def pie(xdata1, ydata1):
    line = (Line()
            .add_xaxis(xdata1)
            .add_yaxis('', ydata1)
            )
    line.set_global_opts(title_opts=opts.TitleOpts(title="最新价"))
    return line
...
pie(xdata1, ydata1).render("pie.html")